...
| Android app (`rrx-app`) | **Toolchain assessed; sensing and crash classification both built and verified** (no local JDK/SDK on this machine — verified via a throwaway Docker image instead, including 16 passing unit tests). Five-module layout per PRD §12.6; `app`, `core-sensing`, and `core-detection` have real code -- device registration, a full Stage-A sensing pipeline, and a TFLite classifier invoked on every Stage-A trigger, verified against the real bundled model artifact (including a NaN bug in the model's audio frontend found and worked around). Transport and data are still placeholder modules — see `android/README.md` |
...
Roughly **62% of the MVP is built** — the ML layer, a functionally complete backend (schema, ingest, risk serving, SMS, WebSocket, sim endpoints, all verified against real Docker containers), a working Live Operations dashboard wired end-to-end to that backend (including a real-time WS path verified live), and now an Android app that senses driving, evaluates Stage-A, and runs a real on-device classification when it fires -- verified end-to-end against the actual trained model, not just compiled. What's left is transport, the cancel-window screen, and onboarding on Android (~14.5 person-days) plus the smaller remainders in each other track.
...
**Toolchain assessed and scaffold built 2026-08-18; real IMU/GPS sensing and a
working TFLite classifier landed the same day.** This dev machine has no
...
produced a real, installable `app-debug.apk` (~26.3 MB, up from ~11.3 MB now
that it bundles the TFLite native runtime and the 300 KB model asset) via
`gradle assembleDebug` -- full chain: Kotlin compile, Hilt/KSP codegen,
resource linking, DEX, packaging, debug signing -- plus 16 passing JVM unit
tests (14 sensing + 2 tabular-feature-extraction, the latter pinning a
hand-computed 26-value example field by field).
...
`CrashClassifier`'s doc comment. See `android/README.md` for the full
...
| **Total** | **~14.5** | down from ~19 — sensing (3 days) and the TFLite runner (1 day) are done, Stage-A gate reduced to just the boot-time remainder |
...
**Grand total ≈ 35.5 person-days** (backend 2.5 + ETL 4 + Android 14.5 + dashboard 7 + integration 7.5), down from ~58. At 5 people over 6 weeks (~150 person-days available) that is comfortable. **Android is still the critical path** — sensing and classification both now work end-to-end (IMU/GPS in, a real Stage-A-triggered TFLite score out), but transport, the cancel-window screen, and onboarding are the bulk of what's left.
...
Week 3  android: cancel window + transport               <- now the whole week's work
