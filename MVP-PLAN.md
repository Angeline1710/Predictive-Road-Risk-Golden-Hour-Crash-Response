...
Roughly **90% of the MVP is built** — the ML layer, a functionally complete backend (schema, ingest, risk serving with condition-simulator overrides and a batched 24h×7d heat-grid endpoint, SMS, WebSocket, sim, and analytics-aggregate endpoints, all verified against real Docker containers), a fully-built dashboard (Live Operations, Incident Detail, Risk Map -- including Corridor mode -- Simulator Console, Analytics) wired end-to-end to that backend and verified live throughout, and an Android app whose sensing, on-device classification, transport, cancel-window screen, onboarding flow, Drive Mode map, §14's Risk Warning, and now the ambient notification all work end to end and are each verified against something real. What's left on Android is just a Settings screen (~0.5 person-days); integration/hardening (~7.5 days) is now the single largest remaining track by a wide margin. Risk Map's Comparison mode is not scheduled at all — it's blocked on a real blackspot dataset this project confirmed it doesn't have and can't honestly fabricate, not on engineering time.
...
**Grand total ≈ 14.5 person-days** (backend 2.5 + ETL 4 + Android 0.5 + dashboard 0 + integration 7.5), down from ~58. At 5 people over 6 weeks (~150 person-days available) that is comfortable. Android and the dashboard are both now built end-to-end and verified against something real: sensing, classification, transport, the cancel-window screen (§15-17), onboarding (§11), Drive Mode's map + Segment Ribbon (§13), Risk Warning (§14 -- voice/haptic/Severe overlay firing off the live segment stream, gated by the real FR-4.4/4.5 cooldown and speed floor), and now a real ambient notification (app-module code updating core-sensing's foreground-service notification by id, respecting PRD §12.6's module boundary rather than crossing it) on the Android side; Incident Detail (§22), Risk Map (§23, including Corridor mode's real batched heat-grid), Simulator Console (§25), and Analytics (§24) on the dashboard side, each extending or adding a real backend endpoint rather than building a frontend against fabricated data. What's left on Android is just a Settings screen (~0.5 days); integration/hardening (7.5 days) is now the largest remaining track by a wide margin -- fifteen times the size of everything else combined. Risk Map's Comparison mode is deliberately excluded from this total -- it's blocked on acquiring a real blackspot dataset, confirmed unavailable anywhere in this project, not on more of this team's engineering time.
