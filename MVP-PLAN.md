...
Roughly **91% of the MVP is built** — the ML layer, a functionally complete backend (schema, ingest, risk serving with condition-simulator overrides and a batched 24h×7d heat-grid endpoint, SMS, WebSocket, sim, and analytics-aggregate endpoints, all verified against real Docker containers, and now real k6-load-tested and one real concurrency bug fixed as a result), a fully-built dashboard (Live Operations, Incident Detail, Risk Map -- including Corridor mode -- Simulator Console, Analytics) wired end-to-end to that backend and verified live throughout, and an Android app whose sensing, on-device classification, transport, cancel-window screen, onboarding flow, Drive Mode map, §14's Risk Warning, and ambient notification all work end to end and are each verified against something real. What's left on Android is just a Settings screen (~0.5 person-days); integration/hardening (~6 days) is now the single largest remaining track by a wide margin, though 1.5 of those days (battery profiling, the shake rig) are flagged as needing real physical hardware this environment doesn't have, not more engineering time here. Risk Map's Comparison mode is not scheduled at all — it's blocked on a real blackspot dataset this project confirmed it doesn't have and can't honestly fabricate, not on engineering time.
...
| Playwright E2E demo script + rehearsal | 1.5 | Not started. The one candidate in this list that's genuinely just software -- a real browser-driven script through PRD §16.2's seven-step jury walkthrough |
...
| **Total** | **~6** | down from ~7.5 — the k6 load test is done and, unusually for this line item, DATA-channel latency instrumentation came along with it for free (same test, same real numbers), reducing that line's own remaining estimate too |

**Grand total ≈ 13 person-days** (backend 2.5 + ETL 4 + Android 0.5 + dashboard 0 + integration 6), down from ~58. At 5 people over 6 weeks (~150 person-days available) that is comfortable. Android and the dashboard are both now built end-to-end and verified against something real: sensing, classification, transport, the cancel-window screen (§15-17), onboarding (§11), Drive Mode's map + Segment Ribbon (§13), Risk Warning (§14 -- voice/haptic/Severe overlay firing off the live segment stream, gated by the real FR-4.4/4.5 cooldown and speed floor), and a real ambient notification (app-module code updating core-sensing's foreground-service notification by id, respecting PRD §12.6's module boundary rather than crossing it) on the Android side; Incident Detail (§22), Risk Map (§23, including Corridor mode's real batched heat-grid), Simulator Console (§25), and Analytics (§24) on the dashboard side, each extending or adding a real backend endpoint rather than building a frontend against fabricated data. The backend itself is now real-load-tested too: a k6 burst test found and led to fixing a genuine event-loop-blocking bug (§3.5), and confirmed NFR-P4's DATA channel, NFR-P5, and NFR-P7 all pass in practice. What's left on Android is just a Settings screen (~0.5 days); integration/hardening (6 days) is now the largest remaining track by a wide margin, though a quarter of that (battery profiling, the shake rig) needs real hardware access this environment can't provide, not more coding. Risk Map's Comparison mode is deliberately excluded from this total -- it's blocked on acquiring a real blackspot dataset, confirmed unavailable anywhere in this project, not on more of this team's engineering time.
