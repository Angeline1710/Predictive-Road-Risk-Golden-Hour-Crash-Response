...
Roughly **93% of the MVP is built** — the ML layer, a functionally complete backend (schema, ingest, risk serving with condition-simulator overrides and a batched 24h×7d heat-grid endpoint, SMS, WebSocket, sim, and analytics-aggregate endpoints, all verified against real Docker containers, and now real k6-load-tested and one real concurrency bug fixed as a result), a fully-built dashboard (Live Operations, Incident Detail, Risk Map -- including Corridor mode -- Simulator Console, Analytics) wired end-to-end to that backend and verified live throughout, an Android app whose sensing, on-device classification, transport, cancel-window screen, onboarding flow, Drive Mode map, §14's Risk Warning, and ambient notification all work end to end and are each verified against something real, and now a full 7-step Playwright E2E rehearsal of PRD §16.2's own jury demo script, all passing live. What's left on Android is just a Settings screen (~0.5 person-days); integration/hardening (~4.5 days) is now the single largest remaining track by a wide margin, though a third of that (battery profiling, the shake rig) is flagged as needing real physical hardware this environment doesn't have, not more engineering time here. Risk Map's Comparison mode is not scheduled at all — it's blocked on a real blackspot dataset this project confirmed it doesn't have and can't honestly fabricate, not on engineering time.
...
| ~~Drive mode + Segment Ribbon + risk warnings + ambient notification~~ | ~~1~~ **0** | **All four sub-pieces done now, the last two verified via Docker (2026-08-25, real `assembleDebug` + `compileDebugKotlin` + `testDebugUnitTest`).** Map + Segment Ribbon (2026-08-21). §14 Risk Warning: `DriveModeViewModel` detects "entering" a High/Severe segment as its nearest-segment id changing (no separate state machine), gated by FR-4.5 (25 km/h floor) and FR-4.4 (15-min per-segment cooldown); `RiskWarningController` speaks the top SHAP factor (FR-4.6) via `TextToSpeech` on `USAGE_ASSISTANCE_NAVIGATION_GUIDANCE` (audible normally, silent when the phone is silenced -- deliberately not the crash siren's DND-overriding `STREAM_ALARM`); Severe adds a double haptic pulse plus a bottom-anchored overlay with a real cross-hatch texture and a live nearest-vertex distance countdown. Caught a real bug: editing `AndroidManifest.xml`'s permissions comment reintroduced the XML `--`-in-comment bug for a **fifth** time, fixed the same way. Ambient notification: `DriveViewModel` (app module) now posts real content -- `● Active · 68 km/h · Chengalpattu · Next: Severe in 1.2 km` -- into the SAME notification `DriveSensingService` (core-sensing module) created via `startForeground()`, updating a foreground service's notification by id from elsewhere in the process (a supported pattern) rather than moving network code into core-sensing and breaking PRD §12.6's module boundary. Polls `/risk/bbox` on a much looser 800m/45s throttle than Drive Mode's on-screen 300m/15s, since this now runs continuously in the background whenever sensing is active, not just while a screen is open. One real gap: the notification's *text* states the band but isn't visually re-coloured -- `NotificationCompat` has no reliable per-notification background-colour API outside `MediaStyle`/`CallStyle` |
...
| **Total** | **~0.5** | down from ~19 — sensing (3 days), the TFLite runner (1 day), transport (2.5 days), the cancel-window screen (2.5 days), Sending/Sent/Acknowledged (1.5 days), onboarding (3 days), battery-optimisation (1 day), and Drive Mode's map + Segment Ribbon + Risk Warning + ambient notification (all 3 of its days) are all done; only the Settings screen itself remains, now that its storage layer exists |

### 3.4 Dashboard — `rrx-ops`

**Started and Live Operations shipped 2026-08-18; Incident Detail, Risk Map, Simulator Console, and Analytics shipped 2026-08-24; Risk Map's Corridor mode shipped 2026-08-25** — React 18 + TS + Vite + Tailwind + react-leaflet + TanStack Query + Zustand per PRD §12.4, verified against the real running backend (not a mock API), including a live WebSocket update, a real curl'd-alert opened end-to-end from a rail card through to a rendered three-column detail view, a live condition-simulator round-trip that measurably moved a real segment's score and band, a live inject-crash round trip that opened the resulting alert's real Incident Detail page, Analytics rendering real aggregate numbers (computed from those same injected alerts) across all six §24 panels with working PNG/CSV export, and Corridor mode's real 168-cell heat-grid genuinely re-fetching when a different ribbon segment is clicked.
...
| ~~Corridor mode~~ | ~~0.5~~ **0** | **Done and verified live, 2026-08-25.** New `GET /risk/heatgrid?segment_id=` returns one segment's real 24h×7d grid (168 `risk_model_v1` predictions, batched into a single LightGBM call -- verified at 0.37s, not the ~26s/1000-segment pattern `GET /risk/bbox` has). The dashboard's Corridor mode toggle replaces the map with a full-width Segment Ribbon (reusing the existing signature component) ordered by straight-line distance from the corridor centre -- a documented proxy, not a true route-km marker, same honesty posture as Android's `NearbySegment` -- plus the heat-grid for whichever segment is clicked, live-verified with a real segment-to-segment reselect (heat-grid genuinely refetches and the displayed SEG id changes) |
| Comparison mode (blackspot) | 0.5 | **Confirmed not buildable, not just "not built yet."** Investigated every accident-related file in the project on the hypothesis that it might be "just an ETL script away": `ml/risk_model/ingest.py`'s own `tn_road_accident_dataset_original.csv` is whole-district annual totals only (Chengalpattu = one row, one number, for all of 2021 -- the ingest module's own docstring says outright "it cannot support segment-level prediction, because it has no geometry"), and `indian_roads_dataset.csv` has real per-row lat/lon but was already profiled and rejected elsewhere in this project as synthetic (statistically-independent severity label) and doesn't cover Chengalpattu/NH-45 at all. No file anywhere in the repo contains an actual ingested MoRTH-iRAD or SaveLIFE-ZFC record -- every mention is schema, disabled UI copy, or spec prose. PRD.md's own open question Q6 ("are published iRAD blackspot lists machine-readable for our corridor?") was never answered. Building Comparison mode today would mean fabricating blackspot placements on the map, which this project's honesty principle rules out -- so it stays out of scope until a real dataset is actually obtained, not scheduled as a same-day task |
...
| **Total** | **~0** | down from ~15.5 — every schedulable line item is done. Comparison mode's remaining 0.5 days is not counted here: it's blocked on acquiring a real blackspot dataset this project doesn't have and confirmed can't self-generate, not on more engineering time, so it isn't "0.5 days of work left" in any schedulable sense |

### 3.5 Integration, hardening, demo

| Piece | Days | Notes |
|---|---|---|
| End-to-end wiring + latency instrumentation against NFR-P4 (≤20 s data / ≤90 s SMS) | 1 | **DATA channel latency is now real-measured, and comfortably passes**: `loadtest/alerts_burst.js`'s max observed request latency was 880ms even under a 100/min sustained burst -- ~23x under the 20s budget. What's left: the SMS channel (`POST /ingest/sms`) has no equivalent instrumentation yet, and NFR-P6 (dashboard WS event latency ≤1s) hasn't been measured either -- that one needs a browser-side timer, not a k6 script |
| Battery profiling on 3 device tiers (NFR-B1–B4) | 1.5 | **Not attempted, and can't be from here.** This needs Battery Historian traces from real physical Android hardware across three device tiers -- there is no device or emulator anywhere in this environment (every Android verification this whole project has done is Docker `compileKotlin`/`assembleDebug`, which proves the code builds, not what it costs on a battery). Flagged honestly rather than estimated from nothing |
| ~~k6 load test — 100 alerts/min burst (NFR-P7)~~ | ~~0.5~~ **0** | **Done and verified live, 2026-08-25** (`backend/loadtest/`). Real k6 scripts against the live Docker stack found a genuine NFR-P3 violation under sustained load (`POST /alerts` p95 ~675-693ms vs. the 400ms budget) and root-caused it with log evidence, not guesswork: `docker logs` showed 200/200 requests during the test window hit `enrichment.geocode.degraded` -- every alert's critical path makes a real call to the public Nominatim API, documented in that function's own comment as rate-limited to 1 req/s, well under this test's 1.67 req/s. A separate, real bug was found and fixed in the same pass: `services/alerts.py`'s LightGBM `predict()` call was blocking the process's single event loop (one `uvicorn` worker is deliberate -- `SimulatedPmRahatGateway`'s ticket state is in-process memory a second worker would fragment), serializing concurrent alerts behind whichever one was mid-inference; `asyncio.to_thread()` cut real connection timeouts from 4/201 to 1/201 in a verified rerun. The Nominatim fix (make it non-blocking, or skip under load) is a product call -- `landmark` is a real dispatch-payload field -- so it's documented, not silently patched. `NFR-P5` (risk query ≤300ms p95) was checked too, in the same pass: real p95 = 79.86ms, comfortably passing |
| Shake rig / controlled trigger for the live demo | 1 | Physical hardware build, out of reach in this environment the same way battery profiling is |
| ~~Playwright E2E demo script + rehearsal~~ | ~~1.5~~ **0** | **Done and verified live, 2026-08-25** (`web/e2e/`). All 7 real Chromium test cases pass against the live Docker backend, walking PRD.md §16.2's seven-step jury script: real assertions for steps 1, 3, 4, 6, 7 (condition-simulator re-scoring, live WS-push timing, the Simulation Seal banner, real Analytics panels including the honest Detection-quality gap, and Comparison mode's real disabled-reason tooltip); labelled software proxies for steps 2 and 5, which need physical hardware (a shake rig, airplane mode) Playwright can't touch -- both documented, not silently faked. Getting it green surfaced two real, non-test-script findings, both written up in `web/e2e/README.md`: (1) `GET /risk/bbox` blocks the backend's single event loop with no per-request batching, so overlapping condition-simulator clicks *serialize* rather than run in parallel -- the script now waits for each fetch to settle before firing the next; (2) step 3's real claim is "an already-open dashboard receives a push within 20s," not "a freshly-loaded page renders within 20s" -- the suite opens the dashboard once in `beforeAll` and asserts against that live page, which is what actually matches PRD's own "app running" stage direction and backend/README.md's own WS verification, and it measured under 2 seconds |
| Deck rebuilt against the working system — architecture slide, model params, artifact size all changed since the original deck (§4.3) | 1 | Not started; a slide-authoring task, not code |
| **Total** | **~4.5** | down from ~7.5 — the k6 load test and the Playwright E2E suite are both done, and DATA-channel latency instrumentation came along with the k6 pass for free (same test, same real numbers), reducing that line's own remaining estimate too |

**Grand total ≈ 11.5 person-days** (backend 2.5 + ETL 4 + Android 0.5 + dashboard 0 + integration 4.5), down from ~58. At 5 people over 6 weeks (~150 person-days available) that is comfortable. Android and the dashboard are both now built end-to-end and verified against something real: sensing, classification, transport, the cancel-window screen (§15-17), onboarding (§11), Drive Mode's map + Segment Ribbon (§13), Risk Warning (§14 -- voice/haptic/Severe overlay firing off the live segment stream, gated by the real FR-4.4/4.5 cooldown and speed floor), and a real ambient notification (app-module code updating core-sensing's foreground-service notification by id, respecting PRD §12.6's module boundary rather than crossing it) on the Android side; Incident Detail (§22), Risk Map (§23, including Corridor mode's real batched heat-grid), Simulator Console (§25), and Analytics (§24) on the dashboard side, each extending or adding a real backend endpoint rather than building a frontend against fabricated data. The backend itself is now real-load-tested too: a k6 burst test found and led to fixing a genuine event-loop-blocking bug (§3.5), and confirmed NFR-P4's DATA channel, NFR-P5, and NFR-P7 all pass in practice. And the whole thing now has a real, 7-for-7-passing Playwright rehearsal of PRD §16.2's own jury demo script, which along the way surfaced and documented a second real backend concurrency finding (`GET /risk/bbox` serializes overlapping requests) distinct from the one the k6 pass found. What's left on Android is just a Settings screen (~0.5 days); integration/hardening (4.5 days) is now the largest remaining track by a wide margin, though a third of that (battery profiling, the shake rig) needs real hardware access this environment can't provide, not more coding. Risk Map's Comparison mode is deliberately excluded from this total -- it's blocked on acquiring a real blackspot dataset, confirmed unavailable anywhere in this project, not on more of this team's engineering time.
...
Week 2  [done] enrichment + gateway + WS + risk endpoints + SMS ingest + sim
        ...
        [done] dashboard: risk map corridor mode (§23 -- new GET
               /risk/heatgrid, a real 168-cell grid batched into one
               LightGBM call; investigated Comparison mode and
               confirmed it isn't buildable without fabricating
               blackspot data -- deliberately left out of scope, see
               §3.4)
        [done] android: ambient notification (§13 -- DriveViewModel
               posts real live content into DriveSensingService's
               foreground notification by id from the app module,
               respecting the core-sensing/app boundary; 45s/800m
               throttle vs. Drive Mode's on-screen 15s/300m, tuned
               for continuous background battery cost)

Week 3  android: post-onboarding Settings screen
        [done] backend: k6 load test + DATA-channel latency
               instrumentation (§3.5 -- found and fixed a real
               event-loop-blocking bug via asyncio.to_thread; found
               and documented, not yet fixed, a Nominatim rate-limit
               bottleneck in POST /alerts' critical path)
        [done] dashboard: Playwright E2E rehearsal of PRD §16.2's
               seven-step jury script (§3.5 -- all 7 pass live;
               found and worked around a second real backend
               finding, GET /risk/bbox serializing overlapping
               requests, distinct from the k6 pass's own finding)

Week 4  integration: SMS-channel + dashboard-WS latency instrumentation
        battery profiling (needs real hardware -- see MVP-PLAN.md §3.5)

Week 5  harden, shake rig, rehearse, rebuild deck
```

**Critical path has moved to Android's very last screen and integration.** Android's remaining ~0.5 days (the Settings screen) is the only scheduled feature work left in the entire MVP; backend, Android's core pipeline (now including §14 Risk Warning and the real ambient notification), and the entire dashboard (Live Operations, Incident Detail, Risk Map including Corridor mode, Simulator Console, Analytics) are all built and verified against real infrastructure and each other. Integration/hardening (§3.5's ~4.5 days) is now what determines whether the full PRD §16.2 walkthrough is demo-ready in time, not any remaining feature gap -- and most of that hardening is already real: a k6 burst test found and led to fixing a genuine backend concurrency bug and confirmed NFR-P4 (DATA channel), NFR-P5, and NFR-P7 all pass under real, measured load, and a 7-step Playwright rehearsal of PRD §16.2's own jury script passes live end to end. Risk Map's Comparison mode is the one item on the whole plan that isn't a matter of more work -- it needs a real MoRTH-iRAD/SaveLIFE-ZFC dataset this project has confirmed it doesn't have, and won't schedule until one is actually obtained. Battery profiling and the shake rig are the other two items that aren't a matter of more coding here -- both need real physical hardware this environment doesn't have.
