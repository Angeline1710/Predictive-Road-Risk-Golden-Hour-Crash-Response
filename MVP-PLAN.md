...
| Android app (`rrx-app`) | **Toolchain assessed; sensing, crash classification, transport, the cancel-window screen (§15-17), and onboarding (§11) all built and verified** (no local JDK/SDK on this machine — verified via a throwaway Docker image instead, including 19 passing unit tests and repeated `compileDebugKotlin`+`testDebugUnitTest`+`assembleDebug` passes). The recurring theme across every pass: a real toolchain catches real bugs review alone doesn't -- an XML `--`-in-comment failure recurred *four times* across manifest and string-resource files, a missing `androidx.compose.runtime.getValue` import, a deprecated `ClickableText` API, a dead `.let`-discarded expression, and a logic bug that would have sent returning users back through onboarding every cold start. Five-module layout per PRD §12.6, all real now: `core-sensing` (IMU/GPS/Stage-A), `core-detection` (TFLite classifier, verified against the real bundled artifact), `core-transport` (HTTPS/SMS channel strategy, wire format confirmed with `curl` against the live backend), `core-data` (Room emergency contacts + EncryptedSharedPreferences consent/medical/language storage), and `app` tying it together -- device registration, a full-screen crash countdown a real on-device `CrashPrediction.pCrash` crossing the model's own decision threshold (0.297) launches via a full-screen-intent notification, and a nine-step onboarding flow gating all of it behind real single-permission-per-screen consent, a real contact picker, real per-app language switching across 5 languages, and real (if unverified against actual hardware) battery-optimisation checks. See `android/README.md` for the full scope-decision writeup |
...
Roughly **73% of the MVP is built** — the ML layer, a functionally complete backend (schema, ingest, risk serving, SMS, WebSocket, sim endpoints, all verified against real Docker containers), a working Live Operations dashboard wired end-to-end to that backend (including a real-time WS path verified live), and now an Android app whose sensing, on-device classification, transport, cancel-window screen, and onboarding flow all work end to end and are each verified against something real -- the trained model artifact, the live backend, or a real Docker compile+test+assemble pass. What's left on Android is Drive Mode's real UI and a post-onboarding Settings screen (~3 person-days) plus the smaller remainders in each other track.
...
| **Total** | **~3** | down from ~19 — sensing (3 days), the TFLite runner (1 day), transport (2.5 days), the cancel-window screen (2.5 days), Sending/Sent/Acknowledged (1.5 days), onboarding (3 days), and battery-optimisation (1 day) are all done; Stage-A gate reduced to just the boot-time remainder, Settings reduced to the screen itself now that its storage layer exists |
...
**Grand total ≈ 24 person-days** (backend 2.5 + ETL 4 + Android 3 + dashboard 7 + integration 7.5), down from ~58. At 5 people over 6 weeks (~150 person-days available) that is comfortable. **Android is still the critical path** — sensing, classification, transport, the cancel-window screen (§15-17), and now onboarding (§11) all work end-to-end; the cancel-window screen produces a real (non-simulated) alert payload from a real on-device trigger, and onboarding is what actually gates a first-time user into that whole pipeline with real consent, not a skip button. Drive Mode's real UI (map, Segment Ribbon, risk warnings) and a post-onboarding Settings screen are the bulk of what's left.
...
Week 3  android: drive mode + risk warnings (real UI, not the debug readout)
