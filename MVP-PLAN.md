...
Roughly **82% of the MVP is built** — the ML layer, a functionally complete backend (schema, ingest, risk serving with condition-simulator overrides, SMS, WebSocket, sim endpoints, and now a fuller alert-detail endpoint, all verified against real Docker containers), a Live Operations + Incident Detail + Risk Map dashboard wired end-to-end to that backend (including a real-time WS path, a real curl'd-alert-to-rendered-detail-page path, and a live what-if re-scoring loop, all verified live), and an Android app whose sensing, on-device classification, transport, cancel-window screen, onboarding flow, and Drive Mode map all work end to end and are each verified against something real. What's left on Android is §14's Risk Warning, the ambient notification, and a Settings screen (~1.5 person-days); the dashboard's Risk Map corridor/comparison modes + Analytics + Simulator console (~3.5 days) and integration/hardening (~7.5 days) are now the largest remaining tracks.
...
**Started and Live Operations shipped 2026-08-18, Incident Detail and Risk Map both shipped 2026-08-24** — React 18 + TS + Vite + Tailwind + react-leaflet + TanStack Query + Zustand per PRD §12.4, verified against the real running backend (not a mock API), including a live WebSocket update, a real curl'd-alert opened end-to-end from a rail card through to a rendered three-column detail view, and a live condition-simulator round-trip that measurably moved a real segment's score and band.
...
| **Total** | **~3.5** | down from ~15.5 — Incident Detail (2 days) and the Risk Map's map/simulator/table (1.5 of its 2.5 days) are done |
...
**Grand total ≈ 19 person-days** (backend 2.5 + ETL 4 + Android 1.5 + dashboard 3.5 + integration 7.5), down from ~58. At 5 people over 6 weeks (~150 person-days available) that is comfortable. **The critical path has moved off Android** for the first time this project: sensing, classification, transport, the cancel-window screen (§15-17), onboarding (§11), and Drive Mode's map + Segment Ribbon (§13) all work end-to-end, each verified against something real -- and the dashboard has now shipped Incident Detail (§22) and Risk Map (§23) the same way: Incident Detail by extending `GET /alerts/{uuid}` for real, Risk Map by extending `GET /risk/bbox` with real condition-simulator overrides and proving them against a live before/after score change, rather than building a frontend against fabricated data. What's left on Android (§14's Risk Warning, the ambient notification, a Settings screen) is ~1.5 days; the dashboard's Risk Map corridor/comparison modes + Analytics + Simulator console (3.5 days) and integration/hardening (7.5 days) are now the largest remaining tracks.
...
**Critical path has moved to the dashboard's remaining views and integration.** Android's remaining ~1.5 days (Risk Warning, ambient notification, Settings screen) are small enough not to gate anything; backend, Android's core pipeline, and the dashboard's Live Operations + Incident Detail + Risk Map views are all verified against real infrastructure and each other. Risk Map's corridor/comparison modes + Analytics + Simulator console (§3.4's remaining ~3.5 days) and integration/hardening (§3.5's ~7.5 days) are now what determines whether the full PRD §16.2 walkthrough — not just Live Operations — is ready in time.
