...
Roughly **84% of the MVP is built** — the ML layer, a functionally complete backend (schema, ingest, risk serving with condition-simulator overrides, SMS, WebSocket, sim endpoints, and now a fuller alert-detail endpoint, all verified against real Docker containers), a Live Operations + Incident Detail + Risk Map + Simulator Console dashboard wired end-to-end to that backend (including a real-time WS path, a real curl'd-alert-to-rendered-detail-page path, a live what-if re-scoring loop, and a live inject-crash-to-detail-page round trip, all verified live), and an Android app whose sensing, on-device classification, transport, cancel-window screen, onboarding flow, and Drive Mode map all work end to end and are each verified against something real. What's left on Android is §14's Risk Warning, the ambient notification, and a Settings screen (~1.5 person-days); the dashboard's Risk Map corridor/comparison modes + Analytics (~2.5 days) and integration/hardening (~7.5 days) are now the largest remaining tracks.
...
| Drive mode + Segment Ribbon + risk warnings | 1 | **The map + Segment Ribbon half is done and verified via Docker (2026-08-21).** `DriveModeScreen` draws a live `osmdroid` map with risk-banded route polylines from the same `GET /risk/bbox` contract the dashboard uses, plus a Segment Ribbon with real letter-token bands and a SHAP-derived next-notable line -- manually opened from Drive Mode's debug section, not auto-launched on `IN_VEHICLE` (documented scope call, `android/README.md`). What's left: §14's Risk Warning (voice/haptic/Severe overlay on entering a High/Severe segment) and the ambient/screen-off notification upgrade, both unbuilt |
...
| **Total** | **~1.5** | down from ~19 — sensing (3 days), the TFLite runner (1 day), transport (2.5 days), the cancel-window screen (2.5 days), Sending/Sent/Acknowledged (1.5 days), onboarding (3 days), battery-optimisation (1 day), and the Drive Mode map + Segment Ribbon (2 of its 3 days) are all done; Stage-A gate reduced to just the boot-time remainder, Settings reduced to the screen itself now that its storage layer exists, Drive Mode reduced to Risk Warning (§14) + the ambient notification |

### 3.4 Dashboard — `rrx-ops`

**Started and Live Operations shipped 2026-08-18, Incident Detail, Risk Map, and Simulator Console all shipped 2026-08-24** — React 18 + TS + Vite + Tailwind + react-leaflet + TanStack Query + Zustand per PRD §12.4, verified against the real running backend (not a mock API), including a live WebSocket update, a real curl'd-alert opened end-to-end from a rail card through to a rendered three-column detail view, a live condition-simulator round-trip that measurably moved a real segment's score and band, and a live inject-crash round trip that opened the resulting alert's real Incident Detail page.
...
| **Total** | **~2.5** | down from ~15.5 — Incident Detail (2 days), the Risk Map's map/simulator/table (1.5 of its 2.5 days), and Simulator Console (1 day) are done |
...
**Grand total ≈ 18 person-days** (backend 2.5 + ETL 4 + Android 1.5 + dashboard 2.5 + integration 7.5), down from ~58. At 5 people over 6 weeks (~150 person-days available) that is comfortable. **The critical path has moved off Android** for the first time this project: sensing, classification, transport, the cancel-window screen (§15-17), onboarding (§11), and Drive Mode's map + Segment Ribbon (§13) all work end-to-end, each verified against something real -- and the dashboard has now shipped Incident Detail (§22), Risk Map (§23), and Simulator Console (§25) the same way: Incident Detail by extending `GET /alerts/{uuid}` for real, Risk Map by extending `GET /risk/bbox` with real condition-simulator overrides and proving them against a live before/after score change, Simulator Console by wiring its two real `/sim/*` endpoints and proving an inject-crash-to-detail-page round trip, rather than building a frontend against fabricated data or a UI whose buttons don't do anything. What's left on Android (§14's Risk Warning, the ambient notification, a Settings screen) is ~1.5 days; the dashboard's Risk Map corridor/comparison modes + Analytics (2.5 days) and integration/hardening (7.5 days) are now the largest remaining tracks.
...
Week 4  dashboard: analytics + export
...
**Critical path has moved to the dashboard's remaining views and integration.** Android's remaining ~1.5 days (Risk Warning, ambient notification, Settings screen) are small enough not to gate anything; backend, Android's core pipeline, and the dashboard's Live Operations + Incident Detail + Risk Map + Simulator Console views are all verified against real infrastructure and each other. Risk Map's corridor/comparison modes + Analytics (§3.4's remaining ~2.5 days) and integration/hardening (§3.5's ~7.5 days) are now what determines whether the full PRD §16.2 walkthrough — not just Live Operations — is ready in time.
